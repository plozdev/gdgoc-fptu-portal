  ...
  return (
    <aside className="w-64 bg-slate-950 text-slate-200 flex flex-col h-full border-r border-slate-800 select-none shrink-0">
      ...
      <nav className="flex-1 overflow-y-auto px-3 py-4 space-y-5 custom-scrollbar">
        ...
        <div className="space-y-1">
          ...
          >
            ...
            <span className="text-[10px] px-1.5 py-0.2 rounded bg-sky-500/20 text-sky-300 font-mono-code">
              Me
            </span>
          </NavLink>
        </div>
        ...
        <div>
          ...
          <div className="space-y-1">
            {/* Phân quyền Nhân sự: BCN (isOrgAdmin) và Ban HR-Event (Lead & Member) */}
            {(isOrgAdmin || user?.banId === 'hr-event') ? (
              <NavLink
                to="/app/hr"
                className={({ isActive }) =>
                  `flex items-center justify-between px-3 py-2 rounded-xl text-xs font-semibold transition-all ${
              ...
              >
                ...
                <span className="text-[10px] px-1.5 py-0.2 rounded bg-blue-500/20 text-blue-300 border border-blue-500/40 font-mono-code">
                  {isOrgAdmin ? 'Admin' : 'HR'}
                </span>
              </NavLink>
            ) : null}
      ...
      <div className="p-3 border-t border-slate-800 bg-slate-900/80">
        {/* User Profile Card linked to /app/profile */}
        <Link
          to="/app/profile"
          title="Xem và chỉnh sửa hồ sơ cá nhân"
          className="block p-2.5 rounded-xl bg-slate-800/80 hover:bg-slate-800 border border-slate-700/60 hover:border-slate-600 mb-2.5 transition-all group"
        >
          <div className="flex items-center gap-2.5 mb-1.5">
            <div className="w-8 h-8 rounded-full bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center font-bold text-xs text-white shadow-xs group-hover:scale-105 transition-transform">
